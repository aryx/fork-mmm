...
class behaviour (base, formtag, deftarget, i18n_encoder) =
 object (self)
  val mutable (*private*) elem_value = ([] : (Htmlfmt.input_kind * (unit -> (string * string) list)) list)
  val mutable (*private*) elem_reset = ([] : (unit -> unit) list)
  val (*private*) fmethod = String.uppercase_ascii (Html.get_attribute formtag "method")    
  val (*private*) encoding = Html.get_attribute formtag "enctype"
  (* val i18n_encoder = i18n_encoder *)
  val (*private*) action = try Html.get_attribute formtag "action" with Not_found -> base
  (* val base = base *)
  val (*private*) h_params = 
    try ["target", Html.get_attribute formtag "target"]
    with Not_found ->
 ...
 (fun _fo tform ->
  ...
  let open_input (fo : Htmlfmt.formatter) t =
    ...
    with Not_found -> 
      ...
      let fr = fo.create_embedded (Html.get_attribute t "align") None None in
  ...
  and tselect = ref Html.{tag_name = "select"; attributes = []}
  in
    ...
    mach#add_tag "option"
      ...
      mach#push_action 
           ...
           (* the val is by default the "content" of the tag *)
               let va = try Html.get_attribute tag "value" with Not_found -> s in
           options := (va, s,
                          Html.has_attribute tag "selected") :: !options;
           ))
       (fun _ -> mach#pop_action)
  and close_select (fo : Htmlfmt.formatter) =
    mach#remove_tag "option";
    let fr = fo.create_embedded (Html.get_attribute !tselect "align") None None in
  ...
  and close_textarea (fo : Htmlfmt.formatter) =
    ...
    let fr = 
      fo.create_embedded (Html.get_attribute !ttextarea "align") None None in
