...
let rec enter_argtype = function
  ...
  | Product tyl -> List.iter enter_argtype tyl
...
let rec enter_template_types = function
   ...
   | ListArg l -> List.iter enter_template_types l

(* Find type dependancies on s *)
let rec add_dependancies s =
  function
    List ty -> add_dependancies s ty
  | Product tyl -> List.iter (add_dependancies s) tyl
...
let rec add_template_dependancies s = function
   ...
   | ListArg l -> List.iter (add_template_dependancies s) l
...
let rec has_callback = function
   ...
   | TypeArg _ -> false
   | ListArg l -> List.exists has_callback l
...
let rec add_return_type = function
  ...
  | Product tyl -> List.iter add_return_type tyl
...
let separate_components =  List.fold_left add_sort []

let enter_widget name components =
  if Hashtbl.mem module_table name then raise (Duplicate_Definition ("widget/module", name)) else
  let sorted_components = separate_components components in
  List.iter 
       (function 
      	 Constructor, l ->
	   enter_subtype "options" MultipleToken 
      	       	name (List.map (function c -> Full c) l)
       | Command, l -> 
	   List.iter enter_component_types l
       | External, _ -> ()
       )
      sorted_components;
  let commands = 
...
let enter_module name components = 
  ...
  List.iter 
       ...
       | Command, l -> List.iter enter_component_types l
       | External, _ -> ()
       )
      sorted_components;
  let commands = 
